...
<div class="w-full">
    ...
    @if(count($selectedIds) > 0)
        <div class="mb-4 flex items-center justify-between bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-3">
            <p class="text-sm text-rose-600 dark:text-rose-400 font-medium">{{ count($selectedIds) }} pelanggan dipilih</p>
            <button type="button" wire:click="hapusTerpilih" wire:confirm="Yakin menghapus semua pelanggan terpilih?" wire:loading.attr="disabled" wire:target="hapusTerpilih" class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-sm font-semibold rounded-lg">
                Hapus Terpilih
            </button>
        </div>
    @endif
                ...
                <tbody class="divide-y divide-slate-800">
                    @forelse($pelanggan as $c)
                        <tr class="hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group">
                            <td class="px-3 py-4 text-center"><input type="checkbox" value="{{ $c->id }}" wire:model.live="selectedIds" class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"></td>
                            <td class="px-6 py-4 text-slate-800 dark:text-slate-200 text-sm font-medium transition-colors duration-300 ease-in-out">{{ $c->nama }}</td>
                            <td class="px-6 py-4 text-slate-500 dark:text-slate-400 text-sm font-mono transition-colors duration-300 ease-in-out"><a href="tel:{{ $c->telepon }}" class="hover:text-blue-500 transition-colors">{{ $c->telepon ?: '-' }}</a></td>
                            <td class="px-6 py-4 text-slate-500 dark:text-slate-400 text-sm transition-colors duration-300 ease-in-out"><a href="mailto:{{ $c->email }}" class="hover:text-blue-500 transition-colors">{{ $c->email ?: '-' }}</a></td>
                            <td class="px-6 py-4 text-right transition-colors duration-300 ease-in-out">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('pelanggan.edit', $c->id) }}" class="p-2 text-slate-500 dark:text-slate-400 hover:text-blue-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Edit Data">
                                        <i data-lucide="pencil" class="w-4 h-4"></i>
                                    </a>
                                    <button type="button" wire:click="hapus({{ $c->id }})" wire:confirm="Yakin menghapus pelanggan ini secara permanen?" wire:loading.attr="disabled" wire:target="hapus" class="p-2 text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:text-rose-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Hapus Permanen">
                        ...
                        <tr>
                            <td colspan="5" class="py-16 text-center transition-colors duration-300 ease-in-out">
                                <div class="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-200 dark:border-slate-800 transition-colors duration-300 ease-in-out">
                                    <i data-lucide="users" class="w-8 h-8 text-slate-500 transition-colors duration-300 ease-in-out"></i>
                                </div>
                                <p class="text-slate-600 dark:text-slate-300 font-medium mb-1 transition-colors duration-300 ease-in-out">Belum ada Pelanggan</p>
                                <p class="text-slate-500 text-sm transition-colors duration-300 ease-in-out">Silakan tambahkan data konsumen atau relasi Anda.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
