<div class="w-full">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Data Produk</h1>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Kelola daftar seluruh inventaris gudang.</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row w-full md:w-auto gap-3">
            ...
            <div class="flex gap-2">
                <a href="{{ route('produk.import') }}" class="flex-1 inline-flex justify-center items-center px-4 py-2.5 bg-[#21262D] border border-[#30363D] hover:bg-[#30363D] text-slate-900 dark:text-white font-medium rounded-xl transition-colors text-sm whitespace-nowrap">
                    <i data-lucide="file-spreadsheet" class="w-4 h-4 mr-2 text-slate-500 dark:text-slate-400"></i> Import
                </a>
                        ...
                        <tr class="hover:bg-[#21262D] transition-colors group">
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-slate-800 dark:text-slate-200 font-mono text-xs">{{ $p->barcode }}</span>
                                    <span class="text-slate-500 font-mono text-[10px] mt-0.5">{{ $p->sku }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-slate-900 dark:text-white font-medium text-sm block line-clamp-2">{{ $p->name }}</span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="inline-block px-2 py-1 bg-[#0D1117] border border-[#30363D] rounded-md text-slate-500 dark:text-slate-400 text-xs font-mono">{{ $p->location ?: '-' }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('produk.edit', $p->id) }}" class="p-2 text-slate-500 dark:text-slate-400 hover:text-[#58A6FF] hover:bg-[#1F6FEB]/10 rounded-lg transition-colors" title="Edit Data">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                ...
                                <p class="text-slate-700 dark:text-slate-300 font-medium mb-1">Tidak ada produk ditemukan</p>
