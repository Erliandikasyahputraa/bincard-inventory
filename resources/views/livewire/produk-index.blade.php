<div class="w-full">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-[#0A1931] dark:text-white tracking-tight transition-colors duration-300 ease-in-out">Data Produk</h1>
            <p class="text-[#4A7FA7]/70 dark:text-slate-400 text-sm mt-1 transition-colors duration-300 ease-in-out">Kelola daftar seluruh inventaris gudang.</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row w-full md:w-auto gap-3">
            <div class="relative group flex-1 md:w-64">
                ...
                <input type="text" wire:model.live.debounce.300ms="cari" placeholder="Cari nama, barcode, SKU..."
                    class="w-full pl-10 pr-4 py-2.5 bg-[#F6FAFD] dark:bg-[#0D1117] border border-[#B3CFE5]/30 dark:border-[#30363D] hover:border-[#8B949E] rounded-xl text-[#1A3D63] dark:text-slate-200 placeholder-slate-500 focus:bg-white dark:bg-[#161B22] focus:border-[#58A6FF] focus:ring-1 focus:ring-[#58A6FF] outline-none transition-all duration-300 text-sm">
            </div>
            
            <div class="flex gap-2">
                <a href="{{ route('produk.import') }}" class="flex-1 inline-flex justify-center items-center px-4 py-2.5 bg-[#F6FAFD] dark:bg-[#21262D] border border-[#B3CFE5]/30 dark:border-[#30363D] hover:bg-[#30363D] text-[#0A1931] dark:text-white font-medium rounded-xl transition-colors text-sm whitespace-nowrap">
                    <i data-lucide="file-spreadsheet" class="w-4 h-4 mr-2 text-[#4A7FA7]/70 dark:text-slate-400 transition-colors duration-300 ease-in-out"></i> Import
                </a>
                <a href="{{ route('produk.tambah') }}" class="flex-1 inline-flex justify-center items-center px-4 py-2.5 bg-[#238636] hover:bg-[#2EA043] text-white font-bold rounded-xl transition-colors shadow-lg shadow-[#1A3D63]/20 dark:shadow-[#3FB950]/20 text-sm whitespace-nowrap">
                        ...
                        <tr class="hover:bg-[#F6FAFD] dark:bg-[#21262D] transition-colors group">
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-[#1A3D63] dark:text-slate-200 font-mono text-xs transition-colors duration-300 ease-in-out">{{ $p->barcode }}</span>
                                    <span class="text-slate-500 font-mono text-[10px] mt-0.5 transition-colors duration-300 ease-in-out">{{ $p->sku }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-[#0A1931] dark:text-white font-medium text-sm block line-clamp-2 transition-colors duration-300 ease-in-out">{{ $p->name }}</span>
                            </td>
                            <td class="px-6 py-4 text-center transition-colors duration-300 ease-in-out">
                                <div class="inline-flex flex-col items-center">
                                    <span class="text-lg font-bold {{ $p->current_stock <= $p->min_stock ? 'text-rose-400' : 'text-[#58A6FF]' }} transition-colors duration-300 ease-in-out">{{ $p->current_stock }}</span>
                            ...
                            <td class="px-6 py-4 text-center transition-colors duration-300 ease-in-out">
                                <span class="inline-block px-2 py-1 bg-[#F6FAFD] dark:bg-[#0D1117] border border-[#B3CFE5]/30 dark:border-[#30363D] rounded-md text-[#4A7FA7]/70 dark:text-slate-400 text-xs font-mono transition-colors duration-300 ease-in-out">{{ $p->location ?: '-' }}</span>
                            </td>
                            <td class="px-6 py-4 text-right transition-colors duration-300 ease-in-out">
                                <div class="flex justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('produk.edit', $p->id) }}" class="p-2 text-[#4A7FA7]/70 dark:text-slate-400 hover:text-[#58A6FF] hover:bg-[#1F6FEB]/10 rounded-lg transition-colors" title="Edit Data">
                                    ...
                                    <button type="button" wire:click="hapus({{ $p->id }})" wire:confirm="Seluruh riwayat transaksi produk ini (ledger) mungkin akan terpengaruh. Lanjutkan menghapus?"
                                        class="p-2 text-[#4A7FA7]/70 dark:text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors" title="Hapus Permanen">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center transition-colors duration-300 ease-in-out">
                                ...
                                <p class="text-[#4A7FA7] dark:text-slate-300 font-medium mb-1 transition-colors duration-300 ease-in-out">Tidak ada produk ditemukan</p>
