<div class="w-full">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Stock Opname</h1>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Audit kesesuaian sistem dengan fisik gudang.</p>
        </div>
        @if(!$opname)
    ...
    @if($opname)
        <div class="mb-6 p-5 sm:p-6 bg-[#161B22] border border-[#30363D] rounded-2xl shadow-xl flex flex-col gap-4">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('opname.export', $opname->id) }}" class="inline-flex justify-center items-center px-4 py-2.5 bg-[#21262D] border border-[#30363D] hover:bg-[#30363D] text-slate-900 dark:text-white font-medium rounded-xl transition-colors text-sm">
                    ...
                    <tbody class="divide-y divide-[#30363D]">
                        @foreach($opname->details as $d)
                            <tr class="hover:bg-[#21262D] transition-colors group">
                                <td class="px-6 py-4 text-slate-800 dark:text-slate-200 text-sm font-medium">{{ $d->product->name }}</td>
                                <td class="px-6 py-4 text-slate-500 dark:text-slate-400 text-xs font-mono">{{ $d->product->barcode }}</td>
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-block px-3 py-1 bg-[#0D1117] border border-[#30363D] rounded-lg text-slate-700 dark:text-slate-300 font-bold font-mono">{{ $d->stok_sistem }}</span>
                                </td>
                                <td class="px-6 py-4 text-center bg-[#238636]/5">
                                    <input type="number" wire:model.live="stokFisik.{{ $d->product_id }}"
                                        class="w-24 text-center bg-[#0D1117] border border-[#30363D] focus:border-[#58A6FF] rounded-lg shadow-sm text-sm text-slate-900 dark:text-white focus:ring-1 focus:ring-[#58A6FF] placeholder-slate-600 outline-none transition-colors border-2 focus:border-2" min="0" placeholder="0">
                                </td>
    ...
    @else
        <div class="bg-[#161B22] border border-[#30363D] rounded-2xl overflow-hidden shadow-xl mt-6">
            <div class="px-6 py-5 border-b border-[#30363D]">
                <h2 class="text-sm font-bold text-slate-800 dark:text-slate-200">Riwayat Sesi Audit / Opname</h2>
            </div>
