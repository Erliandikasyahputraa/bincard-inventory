<div class="w-full">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-[#0A1931] dark:text-white tracking-tight transition-colors duration-300 ease-in-out">Stock Opname</h1>
            <p class="text-[#4A7FA7]/70 dark:text-slate-400 text-sm mt-1 transition-colors duration-300 ease-in-out">Audit kesesuaian sistem dengan fisik gudang.</p>
        </div>
        @if(!$opname)
    ...
    @if($opname)
        <div class="mb-6 p-5 sm:p-6 bg-white dark:bg-[#161B22] border border-[#B3CFE5]/30 dark:border-[#30363D] rounded-2xl shadow-xl flex flex-col gap-4 transition-colors duration-300 ease-in-out">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    <p class="font-bold text-[#0A1931] dark:text-white text-lg transition-colors duration-300 ease-in-out">Sesi Opname</p>
                    <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider {{ $opname->status == 'selesai' ? 'bg-[#238636]/20 text-[#3FB950]' : 'bg-orange-500/20 text-orange-400' }} transition-colors duration-300 ease-in-out">
                        {{ $opname->status }}
                    </span>
                </div>
                <p class="text-sm text-[#4A7FA7]/70 dark:text-slate-400 mb-1 transition-colors duration-300 ease-in-out">Tanggal: <span class="text-[#1A3D63] dark:text-slate-200 transition-colors duration-300 ease-in-out">{{ $opname->tanggal_opname->format('d M Y') }}</span></p>
            ...
            <div class="flex flex-wrap gap-3">
                <button type="button" wire:click="rekonsiliasi" wire:confirm="Sistem akan otomatis membuat log In/Out untuk menyamakan stok sistem dengan nilai fisik yang anda masukkan. Lanjutkan?"
                    class="inline-flex justify-center items-center px-4 py-2.5 bg-[#238636] hover:bg-[#2EA043] text-white font-bold rounded-xl transition-colors shadow-lg shadow-[#1A3D63]/20 dark:shadow-[#3FB950]/20 text-sm">
                    <i data-lucide="check-square" class="w-4 h-4 mr-2"></i> Rekonsiliasi Hasil
                </button>
                <a href="{{ route('opname.export', $opname->id) }}" class="inline-flex justify-center items-center px-4 py-2.5 bg-[#F6FAFD] dark:bg-[#21262D] border border-[#B3CFE5]/30 dark:border-[#30363D] hover:bg-[#30363D] text-[#0A1931] dark:text-white font-medium rounded-xl transition-colors text-sm">
                ...
                <table class="w-full text-left border-collapse min-w-[800px] transition-colors duration-300 ease-in-out">
                    <thead>
                        <tr class="border-b border-[#B3CFE5]/30 dark:border-[#30363D] bg-[#F6FAFD] dark:bg-[#0D1117] text-[11px] font-bold text-[#4A7FA7]/70 dark:text-slate-400 uppercase tracking-wider transition-colors duration-300 ease-in-out">
                            ...
                            <th class="px-6 py-4 font-semibold text-center bg-[#238636]/5 transition-colors duration-300 ease-in-out">Masukan Aktual (Fisik)</th>
                    ...
                    <tbody class="divide-y divide-[#30363D]">
                        @foreach($opname->details as $d)
                            <tr class="hover:bg-[#F6FAFD] dark:bg-[#21262D] transition-colors group">
                                <td class="px-6 py-4 text-[#1A3D63] dark:text-slate-200 text-sm font-medium transition-colors duration-300 ease-in-out">{{ $d->product->name }}</td>
                                <td class="px-6 py-4 text-[#4A7FA7]/70 dark:text-slate-400 text-xs font-mono transition-colors duration-300 ease-in-out">{{ $d->product->barcode }}</td>
                                <td class="px-6 py-4 text-center transition-colors duration-300 ease-in-out">
                                    <span class="inline-block px-3 py-1 bg-[#F6FAFD] dark:bg-[#0D1117] border border-[#B3CFE5]/30 dark:border-[#30363D] rounded-lg text-[#4A7FA7] dark:text-slate-300 font-bold font-mono transition-colors duration-300 ease-in-out">{{ $d->stok_sistem }}</span>
                                </td>
                                <td class="px-6 py-4 text-center bg-[#238636]/5 transition-colors duration-300 ease-in-out">
                                    <input type="number" wire:model.live="stokFisik.{{ $d->product_id }}"
                                        class="w-24 text-center bg-[#F6FAFD] dark:bg-[#0D1117] border border-[#B3CFE5]/30 dark:border-[#30363D] focus:border-[#58A6FF] rounded-lg shadow-sm text-sm text-[#0A1931] dark:text-white focus:ring-1 focus:ring-[#58A6FF] placeholder-slate-600 outline-none transition-colors border-2 focus:border-2" min="0" placeholder="0">
                                </td>
    ...
    @else
        <div class="bg-white dark:bg-[#161B22] border border-[#B3CFE5]/30 dark:border-[#30363D] rounded-2xl overflow-hidden shadow-xl mt-6 transition-colors duration-300 ease-in-out">
            <div class="px-6 py-5 border-b border-[#B3CFE5]/30 dark:border-[#30363D] transition-colors duration-300 ease-in-out">
                <h2 class="text-sm font-bold text-[#1A3D63] dark:text-slate-200 transition-colors duration-300 ease-in-out">Riwayat Sesi Audit / Opname</h2>
            </div>
                ...
                <table class="w-full text-left border-collapse min-w-[600px] transition-colors duration-300 ease-in-out">
                    ...
                    <tbody class="divide-y divide-[#30363D]">
                        @forelse($daftarOpname as $o)
                            <tr class="hover:bg-[#F6FAFD] dark:bg-[#21262D] transition-colors group">
                                <td class="px-6 py-4 text-[#1A3D63] dark:text-slate-200 text-sm font-medium transition-colors duration-300 ease-in-out">{{ $o->tanggal_opname->format('d M Y') }}</td>
                                <td class="px-6 py-4 text-center transition-colors duration-300 ease-in-out">
                                    <span class="inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider {{ $o->status == 'selesai' ? 'bg-[#238636]/20 text-[#3FB950]' : 'bg-orange-500/20 text-orange-400' }} transition-colors duration-300 ease-in-out">
                                        {{ $o->status }}
                                    </span>
                                </td>
