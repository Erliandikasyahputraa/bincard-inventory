    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-end items-start sm:items-center gap-4">
        @if(!$opname)
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto mt-4 sm:mt-0">
                ...
                <button type="button" wire:click="buatOpname" class="inline-flex justify-center items-center px-4 py-2.5 bg-[#10B981] dark:bg-blue-500 hover:bg-[#388BFD] text-white font-bold rounded-xl transition-colors shadow-lg shadow-[#1F6FEB]/20 text-sm whitespace-nowrap flex-shrink-0">
        ...
        <div class="mb-6 p-5 sm:p-6 bg-white dark:bg-slate-900 card-shadow border border-[#D1D5DB] dark:border-slate-800 rounded-2xl shadow-xl flex flex-col gap-4 transition-colors duration-300 ease-in-out">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider {{ $opname->status == 'selesai' ? 'bg-[#10B981] text-white dark:bg-emerald-500/20 dark:text-[#3FB950]' : 'bg-orange-100 text-orange-600 dark:bg-orange-500/20 dark:text-orange-400' }} transition-colors duration-300 ease-in-out">
                        {{ $opname->status }}
                    </span>
                </div>
            ...
            <div class="flex flex-wrap gap-3">
                <button type="button" wire:click="rekonsiliasi" wire:confirm="Sistem akan otomatis membuat log In/Out untuk menyamakan stok sistem dengan nilai fisik yang anda masukkan. Lanjutkan?"
                    class="inline-flex justify-center items-center px-4 py-2.5 bg-[#10B981] dark:bg-emerald-500 hover:bg-emerald-700 dark:hover:bg-[#10B981] text-white font-bold rounded-xl transition-colors shadow-lg shadow-emerald-600/20 dark:shadow-emerald-500/20 text-sm">
                ...
                <table class="w-full text-left border-collapse min-w-[800px] transition-colors duration-300 ease-in-out">
                    <thead>
                        <tr class="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider transition-colors duration-300 ease-in-out">
                            ...
                            <th class="px-6 py-4 font-semibold text-center bg-[#10B981] text-white dark:bg-emerald-500/5 dark:text-slate-400 transition-colors duration-300 ease-in-out">Masukan Aktual (Fisik)</th>
                    ...
                    <tbody class="divide-y divide-slate-800">
                        @forelse($daftarOpname as $o)
                            <tr wire:key="opname-{{ $o->id }}" class="hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-center transition-colors duration-300 ease-in-out">
                                    <span class="inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider {{ $o->status == 'selesai' ? 'bg-[#10B981] text-white dark:bg-emerald-500/20 dark:text-[#3FB950]' : 'bg-orange-100 text-orange-600 dark:bg-orange-500/20 dark:text-orange-400' }} transition-colors duration-300 ease-in-out">
                                        {{ $o->status }}
                                    </span>
                                </td>
