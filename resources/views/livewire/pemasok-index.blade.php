<div class="w-full">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Data Pemasok</h1>
            <p class="text-slate-400 text-sm mt-1">Kelola data vendor dan supplier barang.</p>
        </div>
                ...
                <tbody class="divide-y divide-[#30363D]">
                    @forelse($pemasok as $s)
                        <tr class="hover:bg-[#21262D] transition-colors group">
                            <td class="px-6 py-4 text-slate-200 text-sm font-medium">{{ $s->nama }}</td>
                            <td class="px-6 py-4 text-slate-400 text-sm font-mono"><a href="tel:{{ $s->telepon }}" class="hover:text-[#58A6FF] transition-colors">{{ $s->telepon ?: '-' }}</a></td>
                            <td class="px-6 py-4 text-slate-400 text-sm"><a href="mailto:{{ $s->email }}" class="hover:text-[#58A6FF] transition-colors">{{ $s->email ?: '-' }}</a></td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('pemasok.edit', $s->id) }}" class="p-2 text-slate-400 hover:text-[#58A6FF] hover:bg-[#1F6FEB]/10 rounded-lg transition-colors" title="Edit Data">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-16 text-center">
                                ...
                                <p class="text-slate-300 font-medium mb-1">Belum ada Pemasok</p>
