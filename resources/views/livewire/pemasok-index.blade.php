<div class="w-full">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-[#0A1931] dark:text-white tracking-tight transition-colors duration-300 ease-in-out">Data Pemasok</h1>
            <p class="text-[#4A7FA7]/70 dark:text-slate-400 text-sm mt-1 transition-colors duration-300 ease-in-out">Kelola data vendor dan supplier barang.</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row w-full sm:w-auto gap-3">
            ...
            <a href="{{ route('pemasok.tambah') }}" class="inline-flex justify-center items-center px-4 py-2.5 bg-[#1F6FEB] hover:bg-[#388BFD] text-white font-bold rounded-xl transition-colors shadow-lg shadow-[#1F6FEB]/20 text-sm whitespace-nowrap">
                ...
                <tbody class="divide-y divide-[#30363D]">
                    @forelse($pemasok as $s)
                        <tr class="hover:bg-[#F6FAFD] dark:bg-[#21262D] transition-colors group">
                            <td class="px-6 py-4 text-[#1A3D63] dark:text-slate-200 text-sm font-medium transition-colors duration-300 ease-in-out">{{ $s->nama }}</td>
                            <td class="px-6 py-4 text-[#4A7FA7]/70 dark:text-slate-400 text-sm font-mono transition-colors duration-300 ease-in-out"><a href="tel:{{ $s->telepon }}" class="hover:text-[#58A6FF] transition-colors">{{ $s->telepon ?: '-' }}</a></td>
                            <td class="px-6 py-4 text-[#4A7FA7]/70 dark:text-slate-400 text-sm transition-colors duration-300 ease-in-out"><a href="mailto:{{ $s->email }}" class="hover:text-[#58A6FF] transition-colors">{{ $s->email ?: '-' }}</a></td>
                            <td class="px-6 py-4 text-right transition-colors duration-300 ease-in-out">
                                <div class="flex justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('pemasok.edit', $s->id) }}" class="p-2 text-[#4A7FA7]/70 dark:text-slate-400 hover:text-[#58A6FF] hover:bg-[#1F6FEB]/10 rounded-lg transition-colors" title="Edit Data">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-16 text-center transition-colors duration-300 ease-in-out">
                                ...
                                <p class="text-[#4A7FA7] dark:text-slate-300 font-medium mb-1 transition-colors duration-300 ease-in-out">Belum ada Pemasok</p>
