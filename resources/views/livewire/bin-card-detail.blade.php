...
<div class="flex flex-col gap-4">
    ...
    <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-3 min-w-0">
            <a href="{{ route('produk.index') }}" wire:navigate
                class="p-2 rounded-xl bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 hover:bg-[#F1F5F9] dark:hover:bg-slate-800 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-all shadow-sm flex-shrink-0"
        ...
        <button wire:click="exportExcel" wire:loading.attr="disabled"
            class="flex-shrink-0 inline-flex items-center gap-2 px-4 py-2.5 text-xs font-bold bg-[#16A34A] hover:bg-green-700 text-white rounded-xl transition-all hover:-translate-y-0.5 disabled:opacity-60"
    ...
    <div class="flex flex-wrap items-center gap-2">
        <div class="inline-flex items-center bg-[#F1F5F9] dark:bg-slate-800 p-1 rounded-xl border border-[#E2E8F0] dark:border-slate-700 gap-0.5">
            @foreach (['this_week' => 'Minggu', 'this_month' => 'Bulan Ini', 'last_3_months' => '3 Bulan', 'all' => 'Semua'] as $key => $label)
                <button type="button" wire:click="applyFilter('{{ $key }}')"
                    class="px-3 py-1.5 text-[11px] font-bold rounded-lg transition-all duration-200
                        {{ $activeFilter === $key
                            ? 'bg-white dark:bg-slate-900 text-[#3B82F6] dark:text-blue-400 shadow-sm border border-[#E2E8F0] dark:border-slate-700'
        ...
        <div class="flex items-center gap-1.5 bg-white dark:bg-slate-800 px-2 py-1 rounded-xl border border-[#E2E8F0] dark:border-slate-700"
            style="box-shadow: 0 1px 2px rgba(0,0,0,0.04)">
...
<div class="bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 rounded-2xl p-5"
    ...
    <div class="flex flex-col lg:flex-row gap-5">
        ...
        <div class="flex items-start gap-4 flex-1 min-w-0">
            <div class="w-14 h-14 rounded-2xl bg-[#DBEAFE] dark:bg-blue-500/10 flex items-center justify-center flex-shrink-0">
                <i data-lucide="package-2" class="w-8 h-8 text-[#3B82F6] dark:text-blue-400"></i>
            </div>
        ...
        {{-- Right: Stat Cards --}}
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 lg:max-w-lg w-full">
...
<div class="flex flex-col lg:flex-row gap-5">
    ...
    <div class="lg:w-64 flex flex-col gap-4 flex-shrink-0">
        ...
        {{-- Ringkasan Periode --}}
        <div class="bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 rounded-2xl p-4"
            style="box-shadow: 0 1px 2px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.06)">
            <h3 class="text-xs font-bold text-[#334155] dark:text-slate-200 mb-3 flex items-center gap-1.5">
                <i data-lucide="bar-chart-2" class="w-3.5 h-3.5 text-[#3B82F6]"></i>
                Ringkasan Periode
            </h3>
            <div class="space-y-2">
                <div class="flex justify-between items-center py-1.5 border-b border-[#E2E8F0] dark:border-slate-800">
                ...
                <div class="flex justify-between items-center py-1.5 border-b border-[#E2E8F0] dark:border-slate-800">
                    ...
                    <span class="text-sm font-extrabold text-[#0F172A] dark:text-white">{{ $transactions->count() }}</span>
                </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 rounded-2xl p-4"
            ...
            <div class="space-y-2">
                ...
                <a href="{{ route('barang-masuk.index') }}" wire:navigate
                    class="flex items-center justify-center gap-2 w-full mt-1 px-3 py-2 text-[11px] font-bold text-[#3B82F6] dark:text-blue-400 bg-[#DBEAFE] dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-xl hover:bg-blue-200/60 dark:hover:bg-blue-500/20 transition-colors">
    ...
    <div class="flex-1 bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 rounded-2xl overflow-hidden min-w-0"
        style="box-shadow: 0 1px 2px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.06)">
        <div class="flex items-center justify-between px-5 py-4 border-b border-[#E2E8F0] dark:border-slate-800">
            <h3 class="text-sm font-bold text-[#334155] dark:text-slate-200 flex items-center gap-2">
                <i data-lucide="list" class="w-4 h-4 text-[#3B82F6]"></i>
                Riwayat Transaksi
                <span class="text-[10px] font-bold px-2 py-0.5 bg-[#DBEAFE] dark:bg-blue-500/10 text-[#3B82F6] dark:text-blue-400 rounded-full">{{ $transactions->count() }}</span>
            </h3>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-xs">
                <thead>
                    <tr class="bg-[#F6F8FB] dark:bg-slate-800/50 border-b border-[#E2E8F0] dark:border-slate-800">
                        ...
                        <th class="px-4 py-3 text-right font-semibold text-[#3B82F6] whitespace-nowrap">Saldo</th>
                ...
                <tbody class="divide-y divide-[#E2E8F0] dark:divide-slate-800">
                    ...
                    <tr class="hover:bg-[#F6F8FB] dark:hover:bg-slate-800/40 transition-colors">
                        ...
                        <td class="px-4 py-3 text-right font-extrabold">
                            <span class="{{ $trx->balance <= 0 ? 'text-[#DC2626] dark:text-red-400' : ($trx->balance <= ($product->min_stock ?? 0) ? 'text-[#D97706] dark:text-amber-400' : 'text-[#3B82F6] dark:text-blue-400') }}">
                                {{ number_format($trx->balance, 0, ',', '.') }}
                            </span>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="flex items-center gap-1.5">
                                <div class="w-6 h-6 rounded-full bg-[#DBEAFE] dark:bg-blue-500/20 text-[#3B82F6] dark:text-blue-400 flex items-center justify-center text-[8px] font-extrabold flex-shrink-0">
