<div class="w-full">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Import Produk (Excel)</h1>
            <p class="text-slate-400 text-sm mt-1">Tambahkan data produk sekaligus melalui file spreadsheet.</p>
        </div>
        <div class="flex flex-col sm:flex-row gap-3">
            <a href="{{ route('produk.template') }}" class="inline-flex justify-center items-center px-4 py-2 bg-[#21262D] border border-[#30363D] hover:bg-[#30363D] text-white font-medium rounded-xl transition-colors text-sm whitespace-nowrap">
    ...
    <div class="bg-[#161B22] border border-[#30363D] rounded-2xl shadow-xl overflow-hidden max-w-3xl">
        <div class="p-6 md:p-8 space-y-6">
            ...
            <form wire:submit="simpan">
                <div class="mb-6">
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Pilih File Excel (.xlsx, .xls, .csv)</label>
                    <input type="file" wire:model="file" accept=".xlsx,.xls,.csv" 
                        class="w-full text-sm text-slate-300 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-[#1F6FEB] file:text-white hover:file:bg-[#388BFD] cursor-pointer bg-[#0D1117] border border-[#30363D] rounded-xl outline-none">
