<div class="w-full">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{{ $pelangganId ? 'Edit Pelanggan' : 'Tambah Pelanggan Baru' }}</h1>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Lengkapi informasi entitas pembeli / tujuan barang.</p>
        </div>
    ...
    <form wire:submit="simpan" class="bg-[#161B22] border border-[#30363D] rounded-2xl shadow-xl overflow-hidden max-w-3xl">
        <div class="p-6 md:p-8 space-y-5">
            <div>
                <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Nama Pelanggan <span class="text-rose-500">*</span></label>
        ...
        <div class="px-6 md:px-8 py-4 bg-[#0D1117] border-t border-[#30363D] flex justify-end gap-3">
            <a href="{{ route('pelanggan.index') }}" class="px-5 py-2.5 border border-[#30363D] hover:bg-[#30363D] text-slate-700 dark:text-slate-300 font-bold rounded-xl transition-colors text-sm">Batal</a>
