<div class="w-full">
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">{{ $pelangganId ? 'Edit Pelanggan' : 'Tambah Pelanggan Baru' }}</h1>
            <p class="text-slate-400 text-sm mt-1">Lengkapi informasi entitas pembeli / tujuan barang.</p>
        </div>
    ...
    <form wire:submit="simpan" class="bg-[#161B22] border border-[#30363D] rounded-2xl shadow-xl overflow-hidden max-w-3xl">
        <div class="p-6 md:p-8 space-y-5">
            <div>
                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Nama Pelanggan <span class="text-rose-500">*</span></label>
            ...
            <div>
                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Alamat Lengkap</label>
                <textarea wire:model="alamat" placeholder="Jalan Raya No. 123..."
                    class="w-full bg-[#0D1117] border border-[#30363D] hover:border-[#8B949E] rounded-xl text-slate-200 placeholder-slate-600 focus:bg-[#161B22] focus:border-[#58A6FF] focus:ring-1 focus:ring-[#58A6FF] outline-none transition-all px-4 py-3" rows="3"></textarea>
            </div>
            
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Nomor Telepon / HP</label>
        ...
        <div class="px-6 md:px-8 py-4 bg-[#0D1117] border-t border-[#30363D] flex justify-end gap-3">
            <a href="{{ route('pelanggan.index') }}" class="px-5 py-2.5 border border-[#30363D] hover:bg-[#30363D] text-slate-300 font-bold rounded-xl transition-colors text-sm">Batal</a>
