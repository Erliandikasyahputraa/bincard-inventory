<div class="max-w-6xl mx-auto pb-12">
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group">
                            <td class="py-4 px-4">
                                <p class="font-bold text-slate-800 dark:text-slate-200 group-hover:text-slate-900 dark:text-white transition-colors">{{ $user->name }}
                                    @if(auth()->id() === $user->id)
                                        <span class="ml-2 px-2 py-0.5 rounded-full bg-blue-600 dark:bg-blue-500/10 text-blue-500 text-[10px] font-bold uppercase tracking-wider transition-colors duration-300 ease-in-out">Anda</span>
                                    @endif
                                </p>
                            ...
                            <td class="py-4 px-4 text-right transition-colors duration-300 ease-in-out">
                                <div class="flex items-center justify-end gap-2">
                                    <button wire:click="openModal({{ $user->id }})" class="p-2 sm:px-3 sm:py-2 bg-blue-600 dark:bg-blue-500 hover:bg-[#388BFD] text-slate-900 dark:text-white rounded-lg transition-colors border border-[#1F6FEB] shadow-lg shadow-[#1F6FEB]/20 flex items-center gap-2" title="Edit / Reset Password">
    ...
    @if($isModalOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 transition-colors duration-300 ease-in-out">
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200 transition-colors duration-300 ease-in-out">
                ...
                <form wire:submit="simpan" class="p-6 space-y-5">
                    ...
                    <div class="pt-6 border-t border-slate-200 dark:border-slate-800/50 flex justify-end gap-3 transition-colors duration-300 ease-in-out">
                        ...
                        <button type="submit" class="px-6 py-2.5 bg-blue-600 dark:bg-blue-500 hover:bg-[#388BFD] text-white font-bold rounded-xl transition-colors shadow-lg shadow-[#1F6FEB]/20 flex items-center" wire:loading.attr="disabled">
