<div x-data="scanBarcode()" x-init="init()" class="max-w-4xl mx-auto">
    <div class="mb-6 text-center lg:text-left transition-colors duration-300 ease-in-out">
        <h1 class="text-2xl font-black text-[#0A1931] dark:text-white tracking-tight transition-colors duration-300 ease-in-out">Scanner Interaktif</h1>
        <p class="text-[#4A7FA7]/70 dark:text-slate-400 text-sm mt-1 transition-colors duration-300 ease-in-out">Arahkan kamera ke barcode/QR. Sistem akan mencari produk secara otomatis.</p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        
        <!-- Scanner Camera Area -->
        <div class="bg-white dark:bg-[#161B22] border border-[#B3CFE5]/30 dark:border-[#30363D] rounded-3xl p-6 shadow-xl relative overflow-hidden flex flex-col items-center transition-colors duration-300 ease-in-out">
        ...
        <div class="space-y-6 flex flex-col">
            ...
            <div class="relative group">
                ...
                <input type="text" wire:model.live.debounce.300ms="barcodeTerpilih" placeholder="Atau ketik Barcode / SKU secara manual..."
                    class="w-full pl-12 pr-4 py-4 bg-[#F6FAFD] dark:bg-[#0D1117] border border-[#B3CFE5]/30 dark:border-[#30363D] hover:border-[#8B949E] rounded-2xl text-[#1A3D63] dark:text-slate-200 placeholder-slate-500 focus:bg-white dark:bg-[#161B22] focus:border-[#58A6FF] focus:ring-1 focus:ring-[#58A6FF] outline-none transition-all duration-300 font-mono text-sm">
            </div>

            <!-- Pre-loading or Idle State (hidden if found/error) -->
            @if(!$produkDitemukan && $barcodeTerpilih === '')
                <div class="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-[#B3CFE5]/30 dark:border-[#30363D] rounded-3xl p-10 text-center opacity-50 transition-colors duration-300 ease-in-out">
                    <i data-lucide="scan-line" class="w-16 h-16 text-slate-500 mb-4 transition-colors duration-300 ease-in-out"></i>
                    <p class="text-sm font-bold text-[#4A7FA7]/70 dark:text-slate-400 tracking-wider uppercase transition-colors duration-300 ease-in-out">Menunggu Hasil Scan...</p>
                </div>
            @endif
            ...
            @if($produkDitemukan)
                <div class="flex-1 bg-gradient-to-br from-[#161B22] to-[#0D1117] border border-[#238636]/30 rounded-3xl p-8 relative overflow-hidden ring-1 ring-[#238636]/50 shadow-[0_0_30px_rgba(35,134,54,0.1)] transition-all animate-in fade-in zoom-in duration-300">
                    <div class="absolute -right-10 -top-10 w-40 h-40 bg-[#238636]/10 rounded-full blur-3xl transition-colors duration-300 ease-in-out"></div>
                    ...
                    <div class="relative z-10">
                        ...
                        <h2 class="text-2xl font-black text-[#0A1931] dark:text-white leading-tight mb-2 transition-colors duration-300 ease-in-out">{{ $produkDitemukan['name'] }}</h2>
                        <div class="flex gap-4 text-xs font-mono text-[#4A7FA7]/70 dark:text-slate-400 mb-8 border-b border-[#B3CFE5]/30 dark:border-[#30363D] pb-6 transition-colors duration-300 ease-in-out">
                            <span>SKU: <span class="text-[#0A1931] dark:text-white transition-colors duration-300 ease-in-out">{{ $produkDitemukan['barcode'] }}</span></span>
                            <span>&bull;</span>
                            <span>Stok Valid: <span class="{{ $produkDitemukan['current_stock'] > 0 ? 'text-[#58A6FF]' : 'text-rose-400' }} text-sm font-black transition-colors duration-300 ease-in-out">{{ $produkDitemukan['current_stock'] }}</span></span>
                        </div>

                        <p class="text-xs text-[#4A7FA7]/70 dark:text-slate-400 font-bold uppercase tracking-wider mb-4 transition-colors duration-300 ease-in-out">Pilih Jenis Transaksi</p>
                        
                        <div class="grid grid-cols-2 gap-4">
                            <!-- Tombol Barang Masuk Seamless -->
                            <a href="{{ route('barang-masuk.index') }}?product_id={{ $produkDitemukan['id'] }}" 
                               class="flex flex-col items-center justify-center p-4 bg-[#F6FAFD] dark:bg-[#0D1117] border border-[#B3CFE5]/30 dark:border-[#30363D] hover:border-[#238636] hover:bg-[#238636]/10 rounded-2xl group transition-all text-center">
                                ...
                                <span class="text-[#0A1931] dark:text-white font-bold text-sm transition-colors duration-300 ease-in-out">Masuk</span>
                            ...
                            <a href="{{ route('barang-keluar.index') }}?product_id={{ $produkDitemukan['id'] }}" 
                               class="flex flex-col items-center justify-center p-4 bg-[#F6FAFD] dark:bg-[#0D1117] border border-[#B3CFE5]/30 dark:border-[#30363D] hover:border-orange-500/50 hover:bg-orange-500/10 rounded-2xl group transition-all text-center">
                                ...
                                <span class="text-[#0A1931] dark:text-white font-bold text-sm transition-colors duration-300 ease-in-out">Keluar</span>
            ...
            @elseif($barcodeTerpilih !== '')
                <div class="flex-1 bg-rose-500/5 border border-rose-500/20 rounded-3xl p-8 flex flex-col items-center justify-center text-center animate-in shake duration-300 transition-colors duration-300 ease-in-out">
                    ...
                    <p class="text-xs text-[#4A7FA7]/70 dark:text-slate-400 transition-colors duration-300 ease-in-out">Barcode/SKU <span class="font-mono text-[#0A1931] dark:text-white bg-black/20 px-1 rounded transition-colors duration-300 ease-in-out">{{ $barcodeTerpilih }}</span> tidak terdaftar di sistem.</p>
                </div>
            @endif
        </div>
    </div>
